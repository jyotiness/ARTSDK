...
CGSize NISizeOfStringWithLabelProperties(NSString *string, CGSize constrainedToSize, UIFont *font, UILineBreakMode lineBreakMode, NSInteger numberOfLines) {
  ...
  if (numberOfLines == 1) {
    size = [string sizeWithFont:font forWidth:constrainedToSize.width lineBreakMode:(NSLineBreakMode)lineBreakMode];

  } else {
    size = [string sizeWithFont:font constrainedToSize:constrainedToSize lineBreakMode:(NSLineBreakMode)lineBreakMode];
    if (numberOfLines > 0) {
